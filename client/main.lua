...
-- FIX #6: Lesbarer Gerätetyp statt Icon-Name in der Geräteliste
local DeviceLabels = {
...
CreateThread(function()
    while true do
        ...
        -- FIX #5: Outline-Farbe wird nach Alarm-Ende korrekt zurückgesetzt
        if not hasAlarm then
            for _, obj in ipairs(spawnedObjects) do
                if DoesEntityExist(obj) then
                    SetEntityDrawOutline(obj, false)
                    SetEntityDrawOutlineColor(255, 255, 255, 0) -- Reset auf transparentes Weiß
                end
            end
        end
    ...
    for _, obj in ipairs(spawnedObjects) do
        if DoesEntityExist(obj) then
            ...
            if state.systemId == sId then
                ...
                -- FIX #6: Lesbares Label und Icon aus DeviceLabels-Tabelle holen
                local deviceInfo = DeviceLabels[model] or { label = 'Unbekannt', icon = 'microchip' }

                table.insert(deviceElements, {
                    title       = zone,
                    -- Jetzt steht z.B. "Typ: Rauchmelder" statt "Typ: wind"
                    description = string.format("Typ: %s | Distanz: %.1fm", deviceInfo.label, #(coords - GetEntityCoords(cache.ped))),
                    ...
                    onSelect    = function()
                        ...
                        Wait(3000)
                        SetEntityDrawOutline(obj, false)
                        SetEntityDrawOutlineColor(255, 255, 255, 0) -- FIX #5: Reset nach Markierung
                    end
                })
            end
        end
...
AddEventHandler('onResourceStop', function(resourceName)
    ...
    -- FIX #2: ghost ist jetzt global (in placement.lua), daher hier erreichbar
