...
-- Basierend auf d4rk_fd_utility/scene.lua & deiner ursprünglichen placement.lua
...
RegisterCommand('install_bma', function(source, args)
    ...
    -- Start-Werte für die Platzierung (wie in deiner scene.lua)
    local h = 0.0 -- Höhe Offset
    local d = 1.5 -- Distanz Offset
    local r = 0.0 -- Rotation Offset (Z-Achse)
    local pitch = 0.0 -- Rotation Offset (X-Achse, wichtig für Deckenmontage!)
    ...
    CreateThread(function()
        while DoesEntityExist(ghost) do
            ...
            -- Steuerungslogik (Angepasst aus d4rk_fd_utility)
            ...
            -- Pitch (ALT + Pfeiltasten) für Wand/Decke
            ...
            -- Berechnung der Position (Matrix-Logik)
            ...
            if IsControlJustPressed(0, 38) then
                ...
                -- Erst nach Platzierung fragen wir die Daten ab (wie in deinem Original)
                local input = lib.inputDialog('BMA Konfiguration', {
                    { type = 'input', label = 'Zone / Raumname', placeholder = 'z.B. Empfang', required = true },
                    { type = 'input', label = 'Systemname (Nur bei Zentrale)', placeholder = 'Gebäude Name' }
                })

                if input then
                    TriggerServerEvent('d4rk_firealert:server:registerDevice', deviceType, finalCoords, finalRot, input[1], input[2])
                    DeleteEntity(ghost)
                    isPlacing = false
                    lib.hideTextUI()
                    break
